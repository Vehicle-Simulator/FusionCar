namespace Example
{
    ...
    public sealed class PlayerInput : NetworkBehaviour, IBeforeUpdate, IBeforeAllTicks, IBeforeTick
    {
        ...
        [Networked(nameof(PlayerInput))] private GameplayInput _lastKnownInput { get; set; }
        ...
        {
            if (Object == null || Object.HasInputAuthority == false || IgnoreInput)
                return;
        ...
        {
            if (HasInputAuthority == false || IgnoreInput) return;
        ...
        private void ProcessStandaloneInput()
        {
            ...
            if (mouse != null)
            {
                ...
                lookRotationDelta = ProcessLookRotationDelta(new Vector2(-mouseDelta.y, mouseDelta.x),
                    _standaloneLookSensitivity);
            ...
            if (keyboard != null)
            {
                ...
                if (Object.HasInputAuthority == true)
                {
                    ...
                    if (keyboard.numpadPlusKey.wasPressedThisFrame == true)
                    {
                        GetComponent<Player>().ToggleSpeedRPC(1);
                    }

                    if (keyboard.numpadMinusKey.wasPressedThisFrame == true)
                    {
                        GetComponent<Player>().ToggleSpeedRPC(-1);
                    }
                }
        ...
        private void ProcessMobileInput()
        {
            ...
            if (_moveTouch != null && _moveTouch.IsActive == true && _moveTouch.GetDelta().Position.IsZero() == false)
            {
                ...
                moveDirection = new Vector2(_moveTouch.GetDelta().Position.x, _moveTouch.GetDelta().Position.y) *
                                screenSizeFactor;
        ...
        private void ProcessVRInput()
        {
            ...
            {
                if (head.TryGetFeatureValue(XRCommonUsages.devicePosition, out Vector3 headPosition) == true)
                {
                    _renderInput.HeadPosition = headPosition;
                }

                if (head.TryGetFeatureValue(XRCommonUsages.deviceRotation, out Quaternion headRotation) == true)
                {
                    _renderInput.HeadRotation = headRotation;
                }
            }
            ...
            {
                if (leftHand.TryGetFeatureValue(XRCommonUsages.triggerButton, out bool leftTriggerButtonUsed) == true)
                {
                    _renderInput.LeftTrigger = leftTriggerButtonUsed;
                }

                if (leftHand.TryGetFeatureValue(XRCommonUsages.devicePosition, out Vector3 leftHandPosition) == true)
                {
                    _renderInput.LeftHandPosition = leftHandPosition;
                }

                if (leftHand.TryGetFeatureValue(XRCommonUsages.deviceRotation, out Quaternion leftHandRotation) == true)
                {
                    _renderInput.LeftHandRotation = leftHandRotation;
                }
            ...
            {
                if (rightHand.TryGetFeatureValue(XRCommonUsages.triggerButton, out bool rightTriggerButtonUsed) == true)
                {
                    _renderInput.RightTrigger = rightTriggerButtonUsed;
                }

                if (rightHand.TryGetFeatureValue(XRCommonUsages.devicePosition, out Vector3 rightHandPosition) == true)
                {
                    _renderInput.RightHandPosition = rightHandPosition;
                }

                if (rightHand.TryGetFeatureValue(XRCommonUsages.deviceRotation, out Quaternion rightHandRotation) ==
                    true)
                {
                    _renderInput.RightHandRotation = rightHandRotation;
                }
