...
#pragma warning disable 0109
    ...
    public sealed partial class KCC : NetworkAreaOfInterestBehaviour, IBeforeAllTicks, IAfterTick
    {
        ...
        /// </summary>
        public bool IsInFixedUpdate => _isFixed == true || (_driver == EKCCDriver.Fusion && Runner.Stage != default) ||
                                       (_driver == EKCCDriver.Unity && Time.inFixedTimeStep == true);
        ...
        [SerializeField] private KCCSettings _settings = new KCCSettings();
        ...
        private bool _parentedMode;

        private static readonly Action<IKCCProcessor, KCC, KCCData> _setInputProperties =
            (processor, kcc, data) => processor.SetInputProperties(kcc, data);

        private static readonly Action<IKCCProcessor, KCC, KCCData> _setDynamicVelocity =
            (processor, kcc, data) => processor.SetDynamicVelocity(kcc, data);

        private static readonly Action<IKCCProcessor, KCC, KCCData> _setKinematicDirection =
            (processor, kcc, data) => processor.SetKinematicDirection(kcc, data);

        private static readonly Action<IKCCProcessor, KCC, KCCData> _setKinematicTangent =
            (processor, kcc, data) => processor.SetKinematicTangent(kcc, data);

        private static readonly Action<IKCCProcessor, KCC, KCCData> _setKinematicSpeed =
            (processor, kcc, data) => processor.SetKinematicSpeed(kcc, data);

        private static readonly Action<IKCCProcessor, KCC, KCCData> _setKinematicVelocity =
            (processor, kcc, data) => processor.SetKinematicVelocity(kcc, data);

        private static readonly Action<IKCCProcessor, KCC, KCCData> _processPhysicsQuery =
            (processor, kcc, data) => processor.ProcessPhysicsQuery(kcc, data);

        private static readonly Action<IKCCProcessor, KCC, KCCData> _onStay = (processor, kcc, data) =>
            processor.OnStay(kcc, data);

        private static readonly Action<IKCCProcessor, KCC, KCCData> _onInterpolate = (processor, kcc, data) =>
            processor.OnInterpolate(kcc, data);

        private static readonly Action<IKCCProcessor, KCC, KCCData, object> _processUserLogic =
            (processor, kcc, data, userData) => processor.ProcessUserLogic(kcc, data, userData);
        ...
        public void SetParent(bool parented, Vector3 position, Quaternion rotation)
        {
            _rigidbody.isKinematic = !parented;
            _rigidbody.detectCollisions = !parented;
        ...
        public void SynchronizeTransform(bool synchronizePosition, bool synchronizeRotation)
        {
            SynchronizeTransform(Data, synchronizePosition, synchronizeRotation,
                IsInFixedUpdate == false && IsProxy == false);
        }
        ...
        public void EnforceFeature(EKCCFeature feature)
        {
            if (_activeStage != EKCCStage.SetInputProperties)
                throw new InvalidOperationException(
                    $"Enforcing features is allowed only during {nameof(EKCCStage.SetInputProperties)} stage!");
        ...
        public void SuppressFeature(EKCCFeature feature)
        {
            if (_activeStage != EKCCStage.SetInputProperties)
                throw new InvalidOperationException(
                    $"Suppressing features is allowed only during {nameof(EKCCStage.SetInputProperties)} stage!");
        ...
        public void Initialize(EKCCDriver driver)
        {
            ...
            if (driver == EKCCDriver.Fusion && _isSpawned == false)
                throw new InvalidOperationException(
                    "KCC cannot be explicitly initialized with Fusion driver before KCC.Spawned()!");
        ...
        private void InitializeFixedUpdate(bool forceReinitialize)
        {
            ...
            int currentTick = _driver == EKCCDriver.Fusion
                ? (int)Runner.Simulation.Tick
                : Mathf.RoundToInt(Time.fixedUnscaledTime / Time.fixedUnscaledDeltaTime);
        ...
        private void OnRenderUpdateInternal()
        {
            ...
            if (_debug.ShowPath == true)
            {
                if (_renderData.Frame == _fixedData.Frame)
                {
                    UnityEngine.Debug.DrawLine(_fixedData.TargetPosition, _renderData.TargetPosition,
                        KCCDebug.FixedToRenderPathColor, _debug.DisplayTime);
                }
                else
                {
                    UnityEngine.Debug.DrawLine(_lastRenderPosition, _renderData.TargetPosition,
                        KCCDebug.PredictionCorrectionColor, _debug.DisplayTime);
                }
            }
            ...
            if (_settings.RenderBehavior == EKCCRenderBehavior.Predict)
            {
                if (_renderData.DeltaTime < 0.00005f)
                {
                    ...
                    _renderData.DesiredPosition =
                        _renderData.BasePosition + extrapolationVelocity * _renderData.DeltaTime;
                ...
                if (currentFixedData.HasTeleported == false)
                {
                    ...
                    if (previousFixedData != null)
                    {
                        ...
                        _renderData.TargetPosition =
                            ...
                            _predictionError;
                        _renderData.LookPitch =
                            Mathf.Lerp(previousFixedData.LookPitch, currentFixedData.LookPitch, alpha);
                        _renderData.LookYaw = KCCMathUtility.InterpolateRange(previousFixedData.LookYaw,
                            currentFixedData.LookYaw, -180.0f, 180.0f, alpha);
                        _renderData.RealSpeed =
                            Mathf.Lerp(previousFixedData.RealSpeed, currentFixedData.RealSpeed, alpha);
                        _renderData.RealVelocity = Vector3.Lerp(previousFixedData.RealVelocity,
                            currentFixedData.RealVelocity, alpha);
                    }
                }
                ...
                if (current != null && _lastRenderTime <= current.Time)
                {
                    for (int i = 0; i < 5; ++i)
                    {
                        ...
                        if (_lastRenderTime >= previous.Time)
                        {
                            ...
                            Vector3 expectedRenderPosition =
                                Vector3.Lerp(previous.TargetPosition, current.TargetPosition, alpha);

                            if (_debug.ShowPath == true)
                            {
                                UnityEngine.Debug.DrawLine(expectedRenderPosition, _lastRenderPosition,
                                    KCCDebug.PredictionErrorColor, _debug.DisplayTime);
                            }
        ...
        private void ProcessPhysicsQuery(KCCData data)
        {
            ...
            {
                OverlapCapsule(_extendedOverlapInfo, data, data.TargetPosition, _settings.Radius, _settings.Height,
                    _settings.Radius, _settings.CollisionLayerMask, QueryTriggerInteraction.Collide);
                ...
                data.TargetPosition = DepenetrateColliders(_extendedOverlapInfo, data, data.BasePosition,
                    data.TargetPosition, data.HasJumped == false, data.MaxMoveSteps, 3);
                ...
                if (data.IsGrounded == false && data.WasGrounded == true && data.HasJumped == false &&
                    data.IsSteppingUp == false && data.WasSteppingUp == false)
                {
        ...
        private Vector3 DepenetrateColliders(KCCOverlapInfo overlapInfo, KCCData data, Vector3 basePosition,
            Vector3 targetPosition, bool probeGrounding, int maxSteps, int resolverIterations)
        {
            ...
            return DepenetrateMultiple(overlapInfo, data, basePosition, targetPosition, probeGrounding, maxSteps,
                resolverIterations);
        }
            ...
            Vector3 targetPosition, bool probeGrounding, int maxSteps)
        {
            ...
            if (maxSteps <= 1)
            {
                hit.HasPenetration = Physics.ComputePenetration(_collider.Collider, targetPosition, Quaternion.identity,
                    hit.Collider, hit.Transform.position, hit.Transform.rotation, out Vector3 direction,
                    out float distance);
            ...
            else
            {
                ...
                while (remainingSteps > 0)
                {
                    ...
                    hit.HasPenetration = Physics.ComputePenetration(_collider.Collider, desiredPosition,
                        Quaternion.identity, hit.Collider, hit.Transform.position, hit.Transform.rotation,
            ...
            {
                bool isGrounded = KCCPhysicsUtility.CheckGround(_collider.Collider, targetPosition, hit.Collider,
                    hit.Transform, _settings.Radius, _settings.Height, _settings.Extent, minGroundDot,
                    out Vector3 checkGroundNormal, out float checkGroundDistance, out bool isWithinExtent);
                if (isGrounded == true)
                {
                    data.IsGrounded = true;
            ...
            Vector3 targetPosition, bool probeGrounding, int maxSteps, int resolverIterations)
        {
            ...
            if (maxSteps <= 1)
            {
                ...
                for (int i = 0; i < overlapInfo.ColliderHitCount; ++i)
                {
                    ...
                    hit.HasPenetration = Physics.ComputePenetration(_collider.Collider, targetPosition,
                        Quaternion.identity, hit.Collider, hit.Transform.position, hit.Transform.rotation,
                        out Vector3 direction, out float distance);
                    if (hit.HasPenetration == false)
                        continue;
                ...
                while (remainingSubSteps > 0)
                {
                    ...
                    for (int i = 0; i < overlapInfo.ColliderHitCount; ++i)
                    {
                        ...
                        bool hasPenetration = Physics.ComputePenetration(_collider.Collider, targetPosition,
                            Quaternion.identity, hit.Collider, hit.Transform.position, hit.Transform.rotation,
                            out Vector3 direction, out float distance);
        ...
        private void CheckTriggersPenetration(KCCOverlapInfo overlapInfo, KCCData data)
        {
            for (int i = 0; i < overlapInfo.TriggerHitCount; ++i)
            {
                ...
                bool hasPenetration = Physics.ComputePenetration(_collider.Collider, data.TargetPosition,
                    Quaternion.identity, hit.Collider, hit.Transform.position, hit.Transform.rotation,
                    out Vector3 direction, out float distance);
        ...
        private void TryStepUp(KCCOverlapInfo overlapInfo, KCCData data)
        {
            ...
            if (data.IsSteppingUp == true)
            {
                ...
                Vector3 correctionDirection = correctionDistance > 0.001f
                ...
                if (OverlapCapsule(_sharedOverlapInfo, data, checkPosition, checkRadius, _settings.Height, 0.0f,
                        _settings.CollisionLayerMask, QueryTriggerInteraction.Ignore) == true)
                {
                ...
                Vector3 combinedCheckDirectionXZ =
                    Vector3.Normalize(desiredCheckDirectionXZ + correctionCheckDirectionXZ);
                ...
                if (SphereCast(_raycastInfo, data, checkPosition + new Vector3(0.0f, checkRadius, 0.0f), Vector3.down,
                        maxStepHeight, checkRadius, _settings.CollisionLayerMask, QueryTriggerInteraction.Ignore) ==
                    true)
                {
                    if (_raycastInfo.ColliderHitCount > 0)
                    {
                        ...
                        maxStepHeight =
                            Mathf.Clamp(maxStepHeight - (checkPosition.y - highestPoint.y) - _settings.Extent, 0.0f,
                ...
                float remainingDistance = Mathf.Clamp((desiredDistance - travelledDistance) * _settings.StepSpeed, 0.0f,
                    maxStepHeight);
            ...
            static bool IsTouchingSlopeOrWallOrHang(KCCOverlapInfo overlapInfo)
            {
                ...
                {
                    KCCOverlapHit hit = overlapInfo.ColliderHits[i];
                    if (hit.IsWithinExtent == true && (hit.CollisionType == ECollisionType.Slope ||
                                                       hit.CollisionType == ECollisionType.Wall ||
                                                       hit.CollisionType == ECollisionType.Hang))
                        return true;
                }
            ...
            static bool IsTouchingGroundMesh(KCCOverlapInfo overlapInfo)
            {
                ...
                {
                    KCCOverlapHit hit = overlapInfo.ColliderHits[i];
                    if (hit.IsWithinExtent == true && hit.CollisionType == ECollisionType.Ground &&
                        (hit.Type == EColliderType.Mesh || hit.Type == EColliderType.Terrain))
                        return true;
                }
        ...
        private void TrySnapToGround(KCCData data)
        {
            ...
            OverlapCapsule(_sharedOverlapInfo, data,
                data.TargetPosition - new Vector3(0.0f, _settings.GroundSnapDistance, 0.0f), _settings.Radius,
                _settings.Height + _settings.GroundSnapDistance, _settings.Radius, _settings.CollisionLayerMask,
            ...
            {
                targetGroundedPosition = DepenetrateColliders(_sharedOverlapInfo, data, targetGroundedPosition,
                    targetGroundedPosition + penetrationPositionDelta, false, 0, 0);
                ...
                if (data.IsGrounded == true)
                {
                    ...
                    _debug.DrawGroundSnapping(data.TargetPosition, targetGroundedPosition, targetSnappedPosition,
                        IsInFixedUpdate);
        ...
        private static void CalculateGroundProperties(KCCData data)
        {
            if (KCCPhysicsUtility.ProjectOnGround(data.GroundNormal, data.GroundNormal.OnlyXZ(),
                    out Vector3 projectedGroundNormal) == true)
            {
            ...
            if (KCCPhysicsUtility.ProjectOnGround(data.GroundNormal, data.DesiredVelocity.OnlyXZ(),
                    out Vector3 projectedDesiredVelocity) == true)
            {
        ...
        private bool OverlapCapsule(KCCOverlapInfo overlapInfo, KCCData data, Vector3 position, float radius,
            float height, float extent, LayerMask layerMask, QueryTriggerInteraction triggerInteraction)
        {
            ...
            int hitColliderCount = GetPhysicsScene().OverlapCapsule(positionDown, positionUp, radius + extent,
        ...
        private bool Raycast(KCCRaycastInfo raycastInfo, KCCData data, Vector3 origin, Vector3 direction,
            float maxDistance, LayerMask layerMask, QueryTriggerInteraction triggerInteraction)
        {
            ...
            int raycastHitCount = GetPhysicsScene()
                .Raycast(origin, direction, raycastHits, maxDistance, layerMask, triggerInteraction);
        ...
        private bool SphereCast(KCCRaycastInfo raycastInfo, KCCData data, Vector3 origin, Vector3 direction,
            float maxDistance, float radius, LayerMask layerMask, QueryTriggerInteraction triggerInteraction)
        {
            raycastInfo.Reset(false);
            ...
            RaycastHit raycastHit;
            RaycastHit[] raycastHits = _raycastHits;
            int raycastHitCount = GetPhysicsScene().SphereCast(origin, radius, direction, raycastHits, maxDistance,
        ...
        private void UpdateHits(KCCData data, KCCOverlapInfo extendedOverlapInfo)
        {
            ...
            {
                OverlapCapsule(_trackOverlapInfo, data, data.TargetPosition, _settings.Radius, _settings.Height,
        ...
        private void AddCollision(KCCData data, Collider collisionCollider)
        {
            ...
            IKCCInteractionProvider interactionProvider =
                collisionObject.GetComponentNoAlloc<IKCCInteractionProvider>();
            ...
            bool useAntiJitter)
        {
            if (synchronizePosition == true)
            {
                ...
                if (useAntiJitter == true && _activeFeatures.Has(EKCCFeature.AntiJitter) == true &&
                ...
                {
                    _transform.localRotation = data.TransformRotation;
        ...
        {
            if (_settings.Shape == EKCCShape.None || _settings.Shape == EKCCShape.Void ||
                (_settings.SpawnColliderOnProxy == false && HasAnyAuthority == false))
            {
                _collider.Destroy();
                return;
            }
        ...
        private void ProcessStage<T>(EKCCStage stage, KCCData data, Action<IKCCProcessor, KCC, KCCData, T> method,
            T userData)
        {
            _activeStage = stage;
